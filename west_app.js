(function () {
    ...
    // State
    let sortField = 'avg_linear_per_km';
    let sortDir = 'desc';
    let filteredData = [];
    ...
    function populateFilters() {
        const states = [...new Set(WEST_MASTER_DATA.map(d => d.state))].sort();
        const regions = [...new Set(WEST_MASTER_DATA.map(d => d.region).filter(Boolean))].sort();
        const sources = [...new Set(WEST_MASTER_DATA.map(d => d.data_source))].sort();
        const species = [...new Set(WEST_MASTER_DATA.map(d => d.species))].sort();
    ...
    function applyFilters() {
        ...
        filteredData = WEST_MASTER_DATA.filter(d => {
        ...
        renderMapMarkers();
        updateLegend();
        filteredCount.textContent = `(${filteredData.length} of ${WEST_MASTER_DATA.length})`;
    }
    ...
    function abbreviateRegion(r) {
        if (!r) return '';
        return r.replace(' National Forest', ' NF')
    ...
    function abbreviateSource(s) {
        if (!s) return '';
        return s.replace('EPA National Rivers and Streams Assessment 2018\u20132019', 'EPA NRSA')
                .replace('EPA NRSA 2018\u201319', 'EPA NRSA');
    }
    ...
    function abbreviateSpecies(s) {
        ...
        return s.replace('Brook Trout', 'BKT')
                ...
                .replace('Cutthroat Trout', 'CTT')
                .replace('Bull Trout', 'BLT')
                .replace('Tiger Trout', 'TGT');
    }

    // ── Map ──
    function initMap() {
        map = L.map('map').setView([43.5, -111.5], 5);
    ...
    function showDetail(d) {
        ...
        const streamSamples = typeof WEST_SAMPLES_DATA !== 'undefined' && WEST_SAMPLES_DATA[d.stream_name];
    ...
    function updateStats() {
        document.getElementById('totalStreams').textContent = WEST_MASTER_DATA.length.toLocaleString();

        const totalSamples = WEST_MASTER_DATA.reduce((sum, d) => sum + (d.num_samples || 0), 0);
        document.getElementById('totalSamples').textContent = totalSamples.toLocaleString();

        const years = WEST_MASTER_DATA.filter(d => d.first_year && d.last_year);
        ...
        const sources = new Set(WEST_MASTER_DATA.map(d => d.data_source));
    ...
    function initControls() {
        ...
        document.getElementById('resetFilters').addEventListener('click', () => {
            ...
            metricSelect.value = 'linear_km';
            selectedStream = null;
            mapHasView = false;
            streamDetail.style.display = 'none';
            sortField = 'avg_linear_per_km';
            sortDir = 'desc';
            updateColumnHeaders();
            applyFilters();
            updateLegend();
    ...
    function renderSourceCards() {
        ...
        grid.innerHTML = WEST_SOURCES.map(s => `
