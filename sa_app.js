(function () {
    ...
    let legendEl = null;
    let markerMap = {};  // stream_name -> marker
    ...
    function populateFilters() {
        const regions = [...new Set(TROUT_DATA.map(d => d.region))].sort();
        const sources = [...new Set(TROUT_DATA.map(d => d.data_source))].sort();
        const species = [...new Set(TROUT_DATA.map(d => d.species))].sort();
    ...
    function applyFilters() {
        ...
        filteredData = TROUT_DATA.filter(d => {
        ...
        renderMapMarkers();
        updateLegend();
        filteredCount.textContent = `(${filteredData.length} of ${TROUT_DATA.length})`;
    }
    ...
    function renderTable() {
        ...
        filteredData.forEach((d, i) => {
            ...
            const estMark = est && !blankDensity ? '<span class="est-badge" title="Estimated from drainage area &amp; assumed 100m reach">~</span>' : '';
            ...
            tr.innerHTML = `
                ...
                <td>${d.state === 'Tennessee' ? 'TN' : d.state === 'North Carolina' ? 'NC' : d.state}</td>
                <td>${escHtml(abbreviateRegion(d.region))}</td>
    ...
    function abbreviateRegion(r) {
        if (!r) return '';
        return r.replace('East Tennessee (Cherokee NF area)', 'Cherokee NF')
            .replace('Great Smoky Mountains NP', 'GRSM')
            .replace('Western North Carolina', 'Western NC')
            .replace('East Tennessee Tailwater', 'TN Tailwater')
            .replace('Middle Tennessee Tailwater', 'TN Tailwater')
            .replace('East Tennessee (Cherokee NF)', 'Cherokee NF')
            .replace('Western NC (French Broad Basin)', 'French Broad')
            .replace('Western NC (Hiwassee Basin)', 'Hiwassee')
            .replace('Western NC (Little Tennessee Basin)', 'Little TN')
            .replace('Western NC (New River Basin)', 'New River')
            .replace('Western NC (Watauga Basin)', 'Watauga');
    }

    function abbreviateSource(s) {
        if (!s) return '';
        return s.replace("Great Smoky Mountain Nat'l Park", 'NPS-GRSM')
            .replace('TN Wildlife Resources Agency', 'TWRA')
            .replace('NC Wildlife Resources Commission', 'NCWRC')
            .replace('NC DEQ Fish Community Assessment', 'NC DEQ')
            .replace('NPS IRMA 3-Pass Depletion 2023', 'NPS 2023')
            .replace('TWRA Electrofishing Survey ', 'TWRA ')
            .replace('TWRA Stocking Program', 'TWRA Stocking')
            .replace('VA Dept. Wildlife Resources', 'VA DWR');
    }
    ...
    // ── Map ──
    function initMap() {
        map = L.map('map').setView([35.6, -83.5], 8);
        ...
        L.marker([36.031084, -82.558119], {
            icon: L.divIcon({
                className: '',
                html: '<div class="homebase-marker">★</div>',
                iconSize: [28, 28],
                iconAnchor: [14, 14],
            })
        }).bindTooltip('Homebase', { permanent: false, direction: 'top' }).addTo(map);
    ...
    function showDetail(d) {
        ...
        const items = [
            ...
            ['Total Fish Observed', d.total_fish_observed ?? 'N/A'],
        ...
        const streamSamples = typeof SA_SAMPLES_DATA !== 'undefined' && SA_SAMPLES_DATA[d.stream_name];
        if (streamSamples && streamSamples.length > 1) {
            const hasDensity = streamSamples.some(s => s.density != null);
            const hasYOY = streamSamples.some(s => s.yoy > 0 || s.adult > 0);
            const hasWidth = streamSamples.some(s => s.width_m);
            sitesDiv.innerHTML = `
                ...
                <table class="samples-table">
                    <thead><tr>
                        <th>Date</th>
                        <th>Total Fish</th>
                        ${hasYOY ? '<th>YOY</th><th>Adult</th>' : ''}
                        ${hasDensity ? '<th>Density/100m²</th>' : ''}
                        ${hasWidth ? '<th>Width (m)</th>' : ''}
                    </tr></thead>
                    <tbody>
                        ${streamSamples.map(s => `<tr>
                            <td>${s.date || s.year || '-'}</td>
                            <td>${s.fish ?? '-'}</td>
                            ${hasYOY ? `<td>${s.yoy ?? '-'}</td><td>${s.adult ?? '-'}</td>` : ''}
                            ${hasDensity ? `<td>${s.density != null ? s.density.toFixed(2) : '-'}</td>` : ''}
                            ${hasWidth ? `<td>${s.width_m ?? '-'}</td>` : ''}
                        </tr>`).join('')}
                    </tbody>
                </table>
                </div>`;
            document.getElementById('btnExpandSamples').addEventListener('click', () =>
                openSamplesModal(d.stream_name, streamSamples, {hasDensity, hasYOY, hasWidth})
            );
        } else {
            sitesDiv.innerHTML = '';
        }
        ...
        // Zoom map to stream if it has coords
    ...
    function updateStats() {
        document.getElementById('totalStreams').textContent = TROUT_DATA.length;

        const totalSamples = TROUT_DATA.reduce((sum, d) => sum + (d.num_samples || 0), 0);
        document.getElementById('totalSamples').textContent = totalSamples.toLocaleString();

        const years = TROUT_DATA.filter(d => d.first_year && d.last_year);
        ...
        const sources = new Set(TROUT_DATA.map(d => d.data_source));
    ...
    function initControls() {
        [regionFilter, sourceFilter, speciesFilter].forEach(el => {
    ...
    // ── Export ──
    function exportCsv() {
        const headers = ['Rank', 'Stream', 'State', 'Region', 'Avg_Density_per_100m2',
            'Best_Site_Density', 'Peak_Sample_Density', 'Density_Estimated',
            'Avg_Fish_per_Sample', 'Total_Fish', 'Samples', 'Years_Sampled',
        ...
        const rows = filteredData.map((d, i) => [
            ...
            d.avg_fish_per_sample, d.total_fish_observed,
        ...
        a.href = url;
        a.download = 'trout_stream_density_export.csv';
        a.click();
        ...
        const sources = [
            {
                ...
                desc: 'Brook trout abundance from 745 sites across southern Appalachia (1958-2021). 7,939 electrofishing samples.',
                url: 'https://doi.org/10.5066/P9DQID6G'
            },
            {
                name: 'NPS IRMA DataStore',
                desc: 'GRSM three-pass depletion fish census surveys. 21 streams with 2023 data. Annual monitoring since 1984.',
                url: 'https://irma.nps.gov/DataStore/'
            },
            {
                name: 'NC DEQ Fish Community',
                desc: '960+ sites since 1990. Backpack electrofishing across French Broad, Hiwassee, Little TN, New, Watauga basins.',
                url: 'https://deq.nc.gov/about/divisions/water-resources/water-resources-science-and-data/water-sciences-home-page/biological-assessment-branch/fish-community-assessment-data'
            },
            {
                name: 'TWRA Fishing Forecasts',
                desc: 'Tennessee tailwater electrofishing survey results. 11 tailwaters with catch rates and 3-year rolling averages.',
                url: 'https://storymaps.arcgis.com/stories/dbb92bdf718f4fd7839bf4b08fb82747'
            },
            {
                name: 'EBTJV Assessment',
                desc: '24,327 TN/NC catchments assessed. 14,945 with brook trout present. Range-wide occupancy data.',
                url: 'https://easternbrooktrout.org/assessment-data'
            },
            {
                name: 'NCWRC',
                desc: 'North Carolina Wildlife Resources Commission. 8,500 km of managed Public Mountain Trout Waters across 26 counties.',
                url: 'https://www.ncwildlife.gov/fishing/trout-fishing-north-carolina'
            },
            {
                name: 'USFS Treesearch',
                desc: 'Forest Service research on trout biomass (0.01-1.60 g/m\u00B2) and production in 13 southern Appalachian streams.',
                url: 'https://research.fs.usda.gov/treesearch/49340'
            },
            {
                name: 'TWRA Wild Trout GIS',
                desc: 'Wild trout stream locations and GIS data for East Tennessee. Downloadable shapefiles and KML.',
                url: 'https://tn-tnmap.opendata.arcgis.com/datasets/TWRA::wildtroutstreams'
            }
        ];
    ...
    function openSamplesModal(streamName, samples, {hasDensity, hasYOY, hasWidth}) {
        ...
        const hasLength = samples.some(s => s.length_m);
        const hasPasses = samples.some(s => s.passes);
        const hasPassDetail = samples.some(s => s.p1y != null);
        ...
        document.getElementById('samplesModalBody').innerHTML = `
            <table class="samples-table">
                <thead><tr>
                    ...
                    <th>Total</th>
                    ${hasYOY ? '<th>YOY</th><th>Adult</th>' : ''}
                    ${hasPassDetail ? '<th>P1 YOY</th><th>P1 Adult</th><th>P2 YOY</th><th>P2 Adult</th><th>P3 YOY</th><th>P3 Adult</th>' : ''}
                    ${hasDensity ? '<th>Density/100m²</th>' : ''}
                    ${hasLength ? '<th>Length (m)</th>' : ''}
                    ${hasWidth ? '<th>Width (m)</th>' : ''}
                    ${hasPasses ? '<th>Passes</th>' : ''}
                    ${hasSrc ? '<th>Source</th>' : ''}
                </tr></thead>
                <tbody>
                    ${samples.map(s => `<tr>
                        ...
                        <td>${s.fish ?? '-'}</td>
                        ${hasYOY ? `<td>${s.yoy ?? '-'}</td><td>${s.adult ?? '-'}</td>` : ''}
                        ${hasPassDetail ? `<td>${s.p1y ?? '-'}</td><td>${s.p1a ?? '-'}</td><td>${s.p2y ?? '-'}</td><td>${s.p2a ?? '-'}</td><td>${s.p3y ?? '-'}</td><td>${s.p3a ?? '-'}</td>` : ''}
                        ${hasDensity ? `<td>${s.density != null ? s.density.toFixed(2) : '-'}</td>` : ''}
                        ${hasLength ? `<td>${s.length_m ?? '-'}</td>` : ''}
                        ${hasWidth ? `<td>${s.width_m ?? '-'}</td>` : ''}
                        ${hasPasses ? `<td>${s.passes ?? '-'}</td>` : ''}
                        ${hasSrc ? `<td>${s.src ?? '-'}</td>` : ''}
                    </tr>`).join('')}
                </tbody>
            </table>`;
