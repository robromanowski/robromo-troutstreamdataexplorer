(function () {
    ...
    function populateFilters() {
        const states = [...new Set(NY_MASTER_DATA.map(d => d.state))].sort();
        const regions = [...new Set(NY_MASTER_DATA.map(d => d.region).filter(Boolean))].sort();
        const sources = [...new Set(NY_MASTER_DATA.map(d => d.data_source))].sort();
        const species = [...new Set(NY_MASTER_DATA.map(d => d.species))].sort();
    ...
    function applyFilters() {
        ...
        filteredData = NY_MASTER_DATA.filter(d => {
        ...
        renderMapMarkers();
        updateLegend();
        filteredCount.textContent = `(${filteredData.length} of ${NY_MASTER_DATA.length})`;
    }
    ...
    // ── Map ──
    function initMap() {
        map = L.map('map').setView([43.5, -74.5], 7);
    ...
    function showDetail(d) {
        ...
        const streamSamples = typeof NY_SAMPLES_DATA !== 'undefined' && NY_SAMPLES_DATA[d.stream_name];
    ...
    function updateStats() {
        document.getElementById('totalStreams').textContent = NY_MASTER_DATA.length.toLocaleString();

        const totalSamples = NY_MASTER_DATA.reduce((sum, d) => sum + (d.num_samples || 0), 0);
        document.getElementById('totalSamples').textContent = totalSamples.toLocaleString();

        const years = NY_MASTER_DATA.filter(d => d.first_year && d.last_year);
        ...
        const sources = new Set(NY_MASTER_DATA.map(d => d.data_source));
    ...
    function renderSourceCards() {
        ...
        grid.innerHTML = NY_SOURCES.map(s => `
