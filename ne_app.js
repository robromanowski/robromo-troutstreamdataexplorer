(function () {
    ...
    function populateFilters() {
        const states = [...new Set(NE_MASTER_DATA.map(d => d.state))].sort();
        const regions = [...new Set(NE_MASTER_DATA.map(d => d.region).filter(Boolean))].sort();
        const sources = [...new Set(NE_MASTER_DATA.map(d => d.data_source))].sort();
        const species = [...new Set(NE_MASTER_DATA.map(d => d.species))].sort();
    ...
    function applyFilters() {
        ...
        filteredData = NE_MASTER_DATA.filter(d => {
        ...
        renderMapMarkers();
        updateLegend();
        filteredCount.textContent = `(${filteredData.length} of ${NE_MASTER_DATA.length})`;
    }
    ...
    // ── Map ──
    function initMap() {
        map = L.map('map').setView([43.5, -71.5], 7);
    ...
    function showDetail(d) {
        ...
        const streamSamples = typeof NE_SAMPLES_DATA !== 'undefined' && NE_SAMPLES_DATA[d.stream_name];
    ...
    function updateStats() {
        document.getElementById('totalStreams').textContent = NE_MASTER_DATA.length.toLocaleString();

        const totalSamples = NE_MASTER_DATA.reduce((sum, d) => sum + (d.num_samples || 0), 0);
        document.getElementById('totalSamples').textContent = totalSamples.toLocaleString();

        const years = NE_MASTER_DATA.filter(d => d.first_year && d.last_year);
        ...
        const sources = new Set(NE_MASTER_DATA.map(d => d.data_source));
    ...
    function renderSourceCards() {
        ...
        grid.innerHTML = NE_SOURCES.map(s => `
